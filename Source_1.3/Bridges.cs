...
namespace RoadsOfTheRim
{
    ...
    public static class Patch_RoadDefGenStep_Place_Place
    {
        ...
        public static void Postfix(ref RoadDefGenStep_Place __instance, Map map, IntVec3 position, TerrainDef rockDef, IntVec3 origin, GenStep_Roads.DistanceElement[,] distance)
        {
            ...
            if (__instance.place == TerrainDefOf.Railroad && (isGoodTerrain(position.GetTerrain(map)) || position.GetTerrain(map).IsWater))
            {
                map.terrainGrid.SetTerrain(position, TerrainDefOf.Railroad);
            }
            if (__instance.place == TerrainDefOf.Glitterrail && (isGoodTerrain(position.GetTerrain(map)) || position.GetTerrain(map).IsWater))
            {
